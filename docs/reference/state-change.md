...
- **PlayFab Multiplayer (PFMP)**: `windows\include\playfab\multiplayer\` holds `PFMultiplayer.h`,
  `PFLobby.h` and `PFMatchmaking.h`. Flat C linkage (`extern "C"`), C++11 syntax.
- **PlayFab Party**: `windows\include\playfab\party\` holds `Party.h` (a C++ class API) over
